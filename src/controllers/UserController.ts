...
export class UserController implements IEntityController{
  ...
  public getByEmail = async (req: Request, res: Response): Promise<void> => {
    ...
    const user = await this.userRepository.find({ where: { email: email },relations: ["role"]});
    if (user.length === 0) {
  ...
  public getById = async (req: Request, res: Response): Promise<void> => {
    ...
    relations: ["role"] });
      if (!user) {
        ResponseHandler.sendErrorResponse(res,StatusCodes.NO_CONTENT, UserController.ERROR_USER_NOT_FOUND_WITH_ID(id));
        return;
      }
